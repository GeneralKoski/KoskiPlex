...
async def ws_voice(ws: WebSocket):
    try:
        await ws.accept()
        print(f"✅ WS Connection accepted: {ws.client}")
...
PRACTICE_PHRASES = [
    ...
    "A chaotic collection of colorful creatures climbed cautiously over the ancient, moss-covered crumbling stone wall in the heart of the enchanted emerald forest.",
    "In the quiet of the night, the shimmering silver moonlight reflected off the crystal-clear waters of the tranquil mountain lake, creating a scene of unparalleled serenity.",
    "Philosophical inquiries into the nature of existence often lead to fascinating discussions about the fundamental relationship between human consciousness and the external physical reality.",
    "The diligent scientist fastidiously observed the intricate chemical reactions occurring within the pressurized glass chambers of the advanced experimental laboratory.",
    "Synchronized swimming requires an extraordinary level of physical coordination, breath control, and precise rhythmic timing to create a visually stunning aquatic performance.",
    "Grand architecture from the Renaissance era often features elaborate marble facades, soaring vaulted ceilings, and breathtakingly detailed frescoes that tell stories of ancient mythology.",
    "Developing sustainable energy solutions is a critical challenge for modern society as we strive to reduce our collective carbon footprint and protect the delicate global ecosystem.",
    "The enthusiastic young traveler journeyed across the vast, undulating sand dunes of the Sahara Desert, marveling at the majestic beauty of the shifting golden landscape.",
    "Quantum mechanics provides a revolutionary framework for understanding the behavior of subatomic particles, challenging our classical intuitions about cause and effect.",
    "A symphony orchestra is composed of dozens of talented musicians who work in perfect harmony to bring the complex compositions of legendary masters to life."
]

@app.get("/practice/phrase")
async def practice_phrase(lang: str = "it"):
    print(f"📝 Requesting new practice phrase for language '{lang}' from LLM...")
    try:
        phrase = generate_practice_phrase(lang=lang)
        print(f"✨ Phrase generated by AI: {phrase[:50]}...")
        return {"phrase": phrase}
    except Exception as e:
        print(f"❌ LLM Phrase generation failed: {e}")
        fallback = random.choice(PRACTICE_PHRASES)
        print(f"⚠️ Using fallback phrase from list: {fallback[:50]}...")
